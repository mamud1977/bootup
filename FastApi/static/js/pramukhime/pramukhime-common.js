...
function languageChangeCallback(data) {
	...
	if (data.language === 'english') {
		...
		// change help image
		$("#pramukhimecharmap").attr('src', pramukhIME.getHelpImage('img/{kb}-{language}.png'));
		// changed detailed help
		$("#pramukhimehelpdetailed").attr('src', pramukhIME.getHelp('help/{kb}-{language}.html'));
...
function saveSettings() {
	...
	var pramukhime_settings = jQuery.parseJSON(localStorage.getItem('pramukhime_settings_450')) || { 'settings': {}, 'selectedLanguage': {} };
	...
	localStorage.setItem('pramukhime_settings_450', JSON.stringify(pramukhime_settings));
}
...
function setupInitialSettings() {
	...
	var pramukhime_settings = jQuery.parseJSON(localStorage.getItem('pramukhime_settings_450'));
...
function resetSettings() {
	if (typeof Storage === 'undefined' || typeof localStorage === 'undefined') {
		...
		localStorage.removeItem('pramukhime_settings_450');
	}
	// reset the settings
