    ...
    if eventname:
        try:
            with get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute("SELECT data FROM events WHERE eventname = ?", (eventname,))
                row = cursor.fetchone()
            if row:
                event_structure = json.loads(row[0])
            else:
...
async def save_event(events: EventStructure, current_user: TokenData = Depends(get_current_user)):
    ...
    try:
        ...
        with get_connection() as conn:   # ensures conn.close() runs automatically
            ...
            cursor.execute(         
                """
                INSERT INTO events (eventname, data)
                VALUES (?, ?)
                ON CONFLICT(eventname) DO UPDATE SET
                    data = excluded.data
                """,
                (events.eventname, json.dumps(events))
            )
